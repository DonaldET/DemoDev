...
public class HackUtil implements Serializable {
	...
	public static final int BYTES_IN_LONG = Long.numberOfLeadingZeros((long) 0) / 8;
	...
	public static byte[] serializableToBytes(final Serializable obj) {
		...
		try {
			...
			throw new IllegalStateException("I/O error serializing: " + ex.getMessage(), ex);
		}
	...
	public static Serializable bytesToSerializable(final byte[] instance) {
		...
		try (ByteArrayInputStream bis = new ByteArrayInputStream(instance)) {
			...
			throw new IllegalStateException("unexpected class: " + ex.getMessage(), ex);
		} catch (final IOException ex) {
	...
	public static Serializable injectLongKey(final long markerKeyValue, final long replacementKeyValue,
		...
		if (idx < 0)
			throw new IllegalArgumentException(
					"markerKeyValue (" + markerKeyValue + ") not found in token (" + token.getClass().getName() + ")");
		...
		final Serializable tkHacked = HackUtil.bytesToSerializable(instance);
