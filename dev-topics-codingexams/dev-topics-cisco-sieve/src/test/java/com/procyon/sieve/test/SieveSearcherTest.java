 ...
 * opportunities.
 */
package com.procyon.sieve.test;
...
{
  private static final List<Integer> valuesList = Arrays.asList(14, 31, 57, 2,
      3, 4, 21, 18, 7, 9, 11, 5, 102, 9, 1, 101);
  ...
  {
    final int[] searchResult = searcher.search(repeats, values, topCount);
    Assert.assertNotNull(searcher.getName() + " got null", searchResult);
    Assert.assertEquals(searcher.getName() + " found wrong count of values",
        collected.length, searchResult.length);

    for (int i = 0; i < collected.length; i++)
      Assert.assertEquals(searcher.getName() + " found wrong value at [" + i
          + "]", collected[i], searchResult[i]);
  }
}
