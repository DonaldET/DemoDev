...
public class SieveDemo {
	...
	public static void main(String[] args) {
		...
		System.out.println("Testing Sieve Search over " + testSize + " Data Elements in [1 .. " + bound + "]");
		System.out.println("");
		System.out.flush();
		...
		searcherID = searcher.getName();
		doATest(runner, searcherID, searcher, 10 * repeatCount, data, topPicks, bound);

		System.out.println("\nAll Tests complete");
	}

	private static void doATest(final SieveSearchRunner runner, String searcherID, final SieveSearcher searcher,
			final int repeatCount, final int[] data, final int topPicks, final int optionalBound) {
		System.out.println("\n  -- Creating a " + searcherID + " searcher and repeating tests " + repeatCount
				+ " times in runner");
		...
		Validate.isTrue(results.length == topPicks, "unexpected result length, expected " + topPicks + ", got ",
				results.length);

		float perExec = (float) elapsed / repeatCount;
		perExec = (float) (Math.rint(perExec * 10.0 + 0.5) / 10.0);
		System.out.println("Found the " + topPicks + " results in " + elapsed + " milliseconds, or " + perExec
				+ " milliseconds per scan, are:");
