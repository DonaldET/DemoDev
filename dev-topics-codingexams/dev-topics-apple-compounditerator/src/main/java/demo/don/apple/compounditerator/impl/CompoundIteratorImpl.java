...
public class CompoundIteratorImpl<E> implements CompoundIterator<E> {
	 ...
	 *
	 * @param iterators the iterators to add to internal list
	 *
	...
	private boolean copyIterator(final Iterator<E>[] iterators) {
		...
		if (traceEnabled) {
			traceOut.println("Number of Iterators added is : " + listOfIterators.size());
		}
	...
	private boolean moveToNextIterator() {
		if (currentIteratorIndex >= (listOfIterators.size() - 1)) {
			if (traceEnabled) {
				traceOut.println("No next. currentIteratorIndex = " + currentIteratorIndex);
			}
			return false;
		}
		...
		if (traceEnabled) {
			traceOut.println("Moving to next iterator, Index = " + currentIteratorIndex + " of "
					+ listOfIterators.size() + " iterators");
		}

		currentIterator = listOfIterators.get(currentIteratorIndex);
		if (traceEnabled) {
			traceOut.println("Moved to next iterator. currentIteratorIndex = " + currentIteratorIndex);
		}
	...
	public E next() {
		...
		throw new NoSuchElementException("No more items left. Call hasNext and check before calling next.");
	}
