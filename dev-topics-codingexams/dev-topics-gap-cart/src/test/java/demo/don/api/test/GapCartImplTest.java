...
public class GapCartImplTest
{
  ...
  public void test010()
  {
    final Map<Character, Integer> internalCart = ((GapCartImpl) gapCart)
        .getCart();
  ...
  public void test011()
  {
    ...
    Assert.assertEquals("internalCart stored count differs", new Integer(1),
        internalCart.get(singleItemSkuKey));
  }
  ...
  private void checkSkuCounts(final String mixedItemSku)
  {
    ...
    final Map<Character, Integer> internalCart = new TreeMap<Character, Integer>(
        ((GapCartImpl) gapCart).getCart());

    final Set<Entry<Character, Integer>> internalEntrySet = internalCart
        .entrySet();
    Assert.assertEquals("Sku count differs", skuCounts.size(),
        internalEntrySet.size());
    int i = 0;
    for (final Entry<Character, Integer> e : internalEntrySet)
    {
      final SkuCount scnt = skuCounts.get(i);
      Assert
          .assertEquals("Sku keys differ for " + i, e.getKey(), scnt.getKey());
      Assert.assertEquals("Sku counts differ for " + i, e.getValue(),
          scnt.getValue());
      i++;
    }
  }
