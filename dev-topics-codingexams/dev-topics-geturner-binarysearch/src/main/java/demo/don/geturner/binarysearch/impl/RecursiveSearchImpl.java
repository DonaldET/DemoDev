 ...
 * opportunities.
 */
package demo.don.geturner.binarysearch.impl;
 ...
 *
 * @param <T> type of sorted array to search
 */
public class RecursiveSearchImpl<T extends Comparable<T>> extends AbstractBinarySearch<T> implements BinarySearch<T> {
	 ...
	 * <code>[start + 0, start + lth - 1]</code>. A zero length array always returns
	 * KEY_NOT_FOUND.
	 * <p>
	 ...
	 */
	@Override
	protected final int findImpl(final T[] array, final T key, final int start, final int lth) {
		...
		while (lth > 0) {
			...
			if (key2this < 0)
				...
				return findImpl(array, key, mid + 1, (2 * half == lth) ? half - 1 : half);
			} else
				return mid;
		}
