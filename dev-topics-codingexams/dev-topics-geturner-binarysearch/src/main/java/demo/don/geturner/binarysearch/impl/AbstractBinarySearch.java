 ...
 *
 * @param <T> type of sorted and searchable array
 */
public abstract class AbstractBinarySearch<T extends Comparable<T>> implements BinarySearch<T> {
	 ...
	 * @param start the starting position <code>[0, lth)</code>.
	 * @param lth   the length of the array to search (non-negative).
	 * @return the index of the matching entry or KEY_NOT_FOUND
	 */
	protected abstract int findImpl(final T[] array, final T key, final int start, final int lth);
}