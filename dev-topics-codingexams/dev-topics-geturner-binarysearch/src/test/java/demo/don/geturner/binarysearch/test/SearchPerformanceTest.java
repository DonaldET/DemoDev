...
public class SearchPerformanceTest extends AbstractPerformanceChecker {
	...
	public void testInstantiation() {
		Assert.assertTrue("classes same", searchItr.getClass() != searchRec.getClass());

		Assert.assertTrue("searchItr not a " + BinarySearch.class, searchItr instanceof BinarySearch);

		Assert.assertTrue("searchRec not a " + BinarySearch.class, searchRec instanceof BinarySearch);
	...
	public void testPerformance() {
		...
		if (display) {
			...
			System.err.println("    Recursive total for " + testCount + " runs is: " + totRec);
			System.err.println("    Variance: " + round2Places(Math.abs(totItr - totRec) / (double) totItr));
		}

		final double faster = round2Places((double) (totRec - totItr) / (double) totItr);

		Assert.assertTrue("Iterative unexpectedly slower by " + faster + ".  Running values are:  ITR: " + totItr
				+ ";  REC: " + totRec, faster >= 0.0);

		final double expectedfaster = 1.00;
		final double allowedVariance = 1.0;
		final double actualVariance = round2Places(Math.abs(faster - expectedfaster));

		Assert.assertTrue(
				"Iterative unexpectedly faster by " + round2Places(faster) + ";  expected: " + expectedfaster
						+ ";  actual: " + actualVariance + ";  which exceeds " + allowedVariance
						+ " variance.  Running values are:  ITR: " + totItr + ";  REC: " + totRec,
