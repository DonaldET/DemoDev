		...
		for (DataRow row : data) {
			...
			if (rowRead < 1) {
				...
				if (colNames == null || colNames.isEmpty())
					throw new IllegalArgumentException("first row of column names is null or empty");
				for (final String colname : colNames)
					if (colname == null || colname.isEmpty())
						throw new IllegalArgumentException("column name in first row of column names is null or empty");
			} else {
