...
import demo.don.amazon.games.recommenders.SimpleRecommender;
...
public class SimpleRecommenderTest {
	...
	public void testGetRankedRecommendationsMany() {
		...
		Assert.assertEquals("recommendations differ for " + customerID, expected, rankedRecommendations);
	}
	...
	private List<ProductID> checkRankedRecommendation(final CustomerID customerID) {
		...
		final List<ProductID> rankedRecommendations = recommender.getRankedRecommendations(customerID);

		Assert.assertNotNull("rankedRecommendations null for " + customerID, rankedRecommendations);
