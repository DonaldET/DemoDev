 ...
 * opportunities.
 */
package demo.don.amazon.games.test;
...
import demo.don.amazon.games.daostrategy.StubDao;
...
public class StubDaoTest {
	...
	public void testCustomerIDs() {
		...
		Assert.assertTrue("test customer id list size small, " + keyListSize, keyListSize > 5);
	}
	...
	public void testLibraryCustomerIDs() {
		...
		Assert.assertFalse("testLibCustomerIDs empty", testLibCustomerIDs.isEmpty());
		final int keyListSize = testLibCustomerIDs.size();
		Assert.assertTrue("test library customer id list size small, " + keyListSize, keyListSize > 5);
	}
	...
	public void testNonEmptyLibraries() {
		...
		for (final CustomerID id : libraryCustomerIDs) {
			...
			Assert.assertEquals("library differs for " + id, library, dao.getLibraryForUser(id));
		}
	}
	...
	public void testNonEmptyFriends() {
		...
		for (final CustomerID id : friendsCustomerIDs) {
			...
			Assert.assertEquals("friends differs for " + id, friends, dao.getFriendsListForUser(id));
		}
	}
	...
	public void testLibraryAndTestHaveIdenticalCustomerIDs() {
		...
		Assert.assertEquals("library list sizes differ", testCustomerSize, libraryCustomerIDs.size());
		...
		for (int i = 0; i < testCustomerSize; i++) {
			...
			if (!customerID.equals(libCustomerID)) {
				dcount++;
				System.err.println(dcount + ". name entry[" + i + "] differs; " + customerID + " <> " + libCustomerID);
			}
		}
	...
	public void testGetFriendsListForUser() {
		...
		for (final CustomerID id : friendedCustomerIDs) {
			...
			Assert.assertNotNull("friendsListForUser null for " + id, friendsListForUser);
			if (testable.contains(id))
				Assert.assertFalse("friendsListForUser empty for " + id, friendsListForUser.isEmpty());
		}
	}
	...
	public void testGetLibraryForUser() {
		...
		for (final CustomerID id : testCustomerIDs) {
			...
			Assert.assertFalse("libraryForUser empty for " + id, libraryForUser.isEmpty());

			final int sz = libraryForUser.size();
			Assert.assertTrue("bad lib size (" + sz + " <= " + lastSize + ") for " + id, sz > lastSize);
			lastSize = sz;
		}
	}
