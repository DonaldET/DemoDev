 ...
 * opportunities.
 */
package demo.amazon.games.daostrategy;
...
{
  public static final List<CustomerID> testIds = Arrays.asList(new CustomerID(
      "Bob"), new CustomerID("Sam"), new CustomerID("Tom"));

  public static final List<CustomerID> customerIds = Arrays.asList(
      new CustomerID("Bob"), // ***
      new CustomerID("Don"), new CustomerID("Harold"), new CustomerID("Jane"),
      new CustomerID("Janice"), new CustomerID("Jim"), new CustomerID(
          "Michelle"), new CustomerID("Sally"), new CustomerID("Sam"), // ***
      new CustomerID("Tom")); // ***

  private static final List<List<CustomerID>> friendsList = Arrays.asList(
      Arrays.asList(new CustomerID("Tom"), new CustomerID("Harold"),
          new CustomerID("Sally"), new CustomerID("Sam")), // ---Bob
      ...
      Arrays.asList(new CustomerID("Jane"), new CustomerID("Tom"),
          new CustomerID("Harold"), new CustomerID("Sally"), new CustomerID(
              "Jim")), // ---Sam
      Arrays.asList(new CustomerID("Bob"), new CustomerID("Don"),
          new CustomerID("Janice"), new CustomerID("Michelle"), new CustomerID(
              "Sally"))); // ---Tom
  ...
  private void setupData()
  {
    if (customerIds.size() != friendsList.size())
      throw new IllegalArgumentException(
          "friends list out of sync, customerIds[" + customerIds.size()
  ...
  public static void main(final String[] argc)
  {
    ...
    for (final CustomerID id : dao.getAllTestCustomerIDs())
      System.out.println(String.format("%-10s",
          customerIds.contains(id) ? new CustomerID(id.getId() + "*") : id)
  ...
  public List<CustomerID> getAllTestCustomerIDs()
  {
    ...
    for (final Iterator<CustomerID> nameItr = names.iterator(); nameItr
        .hasNext();)
