 ...
 * opportunities.
 */
package demo.don.amazon.games.recommenders;
...
import demo.don.amazon.games.Recommend;
...
public class SimpleRecommender implements Recommend {
	 ...
	 *
	 * @param id the customer for which recommendations are created
	 * @return a list of product recommendations ordered from most popular to least
	 *         popular with friends of the customer
	 */
	@Override
	...
	/**
	 * Count all games in all friends libraries. Performance is based on count step.
	 * <p>
	 * <ul>
	 * <li>Space: O(F * L)</li>
	 * <li>Time: O(F * L)</li>
	 * </ul>
	 *
	 * @param id id of customer with friends
	 ...
	 * @param counts the game usage counts to clean
	 */
	private void removeOwnedGames(final CustomerID id, final Map<ProductID, Integer> counts) {
	 ...
	 * </code>
	 *
	 * @param counts counts of usage by product id
	 * @return list of product ids ordered by counts
	 */
	private List<ProductID> extractOrderedRecommendations(final Map<ProductID, Integer> counts) {
		final List<ProductID> recommendations = new ArrayList<ProductID>();
		if (counts == null || counts.isEmpty())
			return recommendations;
		...
		for (WeightedGame recommendation = ordered.poll(); recommendation != null; recommendation = ordered.poll()) {
