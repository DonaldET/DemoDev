...
public class IndexHelper {
	 ...
	 */
	public static final String SYMBOL_STRING = "0123456789" + "ABCDEFGHIJKLMNOPQRSTUVWZYZ"
	 ...
	 */
	public static final int MAX_VALUE = (int) (Math.ceil(Math.pow((RADIX), MAX_DIGITS))) - 1;
	...
	/**
	 * Encode a non-negative binary value into a text value using the default radix
	 *
	 * @param value a non-negative input value to encode as text
	 *
	 ...
	 * identifier
	 *
	 * @param codedInput the non-null and non-empty encoded text value
	 *
	...
	public static int decode(final String codedInput) {
		...
		for (int i = 0; i < codedLth; i++) {
			...
			if (digitPos < 0)
				throw new IllegalArgumentException("unrecognized digit, '" + new String(new char[] { digitCH }) + "'");
	 ...
	 * </p>
	 *
	 * @param src the non-null and non-empty pattern that will be used to locate
