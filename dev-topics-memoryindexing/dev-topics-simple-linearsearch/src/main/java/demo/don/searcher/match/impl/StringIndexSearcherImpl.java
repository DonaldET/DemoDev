...
public class StringIndexSearcherImpl implements IndexSearcher<String> {
	...
	public void setSearchIndex(final SearchIndex<String> searchIndex) {
		...
		Validate.isTrue(searchIndex.getSearchIndex().endsWith(IndexBuilder.NAME_MARK), "no end mark");
	 ...
	 */
	@Override
	public int[] findUsingLike(final String leftMatchPattern, final int maxReturned) {
		...
		while ((matchPos = index.indexOf(pattern, matchPos)) > -1 && matchCount < matchLimit) {
	 ...
	 */
	@Override
	public int[] findUsingContains(final String containsPattern, final int maxReturned) {
		...
		while (matchCount < maxReturned && beginIndex <= boundry) {
			final int associatedIndexPos = index.indexOf(IndexBuilder.INDEX_MARK, beginIndex);
			if (associatedIndexPos < 0)
				throw new IllegalStateException("no '" + IndexBuilder.INDEX_MARK + "' found beyond " + beginIndex);

			final int nextEntryPos = index.indexOf(IndexBuilder.NAME_MARK, associatedIndexPos + 1);
			if (nextEntryPos < 0)
				throw new IllegalStateException(
						"no '" + IndexBuilder.INDEX_MARK + "' found beyond " + associatedIndexPos);
			...
			if (matchPos > -1)
				matches[matchCount++] = IndexHelper.decode(index.substring(associatedIndexPos + 1, nextEntryPos));
	...
	private ExtractIndex getAssociatedIndex(final String index, final int beyondPatternEnd) {
		int associatedIndexPos = index.indexOf(IndexBuilder.INDEX_MARK, beyondPatternEnd);
		if (associatedIndexPos < 0)
			throw new IllegalStateException("malformed string search index, missing " + IndexBuilder.INDEX_MARK
					+ " beyond " + beyondPatternEnd);

		int nextEntryPos = index.indexOf(IndexBuilder.NAME_MARK, associatedIndexPos + 1);
		if (nextEntryPos < 0)
			throw new IllegalStateException("malformed string search index, missing " + IndexBuilder.NAME_MARK
					+ " at end beyond " + associatedIndexPos);

		final int associatedIndex = IndexHelper.decode(index.substring(associatedIndexPos + 1, nextEntryPos));
