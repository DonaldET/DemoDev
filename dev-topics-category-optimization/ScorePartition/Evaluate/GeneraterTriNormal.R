...
cat(paste('\n-- Generate Sorted Normal Distributions, Skewed Right Mode, in:\n  ', script.dir))
...
send_multimodal_out <- function(lth, label, fname) {
  values <- data_prep(label, generate_multi_modal(lth, 0.05, 0.10))
...
trigger_multi_modal(n, label, 'TriNorm1.csv')
trigger_multi_modal(n, label, 'TriNorm2.csv')
trigger_multi_modal(n, label, 'TriNorm3.csv')
