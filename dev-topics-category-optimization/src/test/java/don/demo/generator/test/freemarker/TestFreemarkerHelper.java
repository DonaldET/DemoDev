...
/**
 * Tests Freemarker helper classes used in the Freemarker-based generator.
 * 
 * @author Don
 *
...
public class TestFreemarkerHelper
{
    ...
    {
        final File baseDir = new File(TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL_DIR);
        final ConfigAndLoader cfgLoader = FreemarkerHelper
                .configureFreemarker(FreemarkerHelper.createFileTemplateLoader(baseDir));
        final Configuration cfg = cfgLoader.getConfiguration();
        Assert.assertNotNull("configuration null", cfg);
        Assert.assertEquals("encoding differs", "UTF-8", cfg.getDefaultEncoding());
        Assert.assertEquals("locale differs", "UTF-8", cfg.getEncoding(Locale.US));
    }
    ...
    public void testRetrieveTemplate()
    {
        ...
        Assert.assertNotNull(
                "null template for " + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL
                        + " from " + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL_DIR,
                template);
        ...
        {
            throw new IllegalArgumentException("unable to dump template "
                    + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL + "; message: "
                    + ex.getMessage(), ex);
        }
    }
    ...
    {
        final File tplFile = new File(TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL_DIR,
                TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL);
        ...
        catch (FileNotFoundException ex)
        {
            Assert.fail("I/O error creating reader for "
                    + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL + " from "
                    + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL_DIR + "; message: "
                    + ex.getMessage());
        }

        final String cachedFile = FreemarkerHelper.cacheFile(reader);
        Assert.assertNotNull(
                "NULL cached file for " + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL
                        + " from " + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL_DIR,
                cachedFile);
        Assert.assertFalse(
                "empty cached file for " + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL
                        + " from " + TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_EVALUATION_TPL_DIR,
                cachedFile.isEmpty());

        final String exp_val = TestSimpleModifiedInterpolationTemplateLoader.SIMPLE_TEMPLATE_READ_VAL.trim();
