 ...
 * opportunities.
 */
package don.demo.algo.quadsolve.test;
...
import don.demo.algo.quadsolve.QuadSolver;
...
public class QuadTest {
	...
	public void testSimple1And3Roots() {
		...
		Assert.assertEquals("first root bad", expectedR1, r1, QuadSolver.ACCURACY_FACTOR);
		final float r2 = roots.get(0).floatValue();
		Assert.assertEquals("second root bad", expectedR2, r2, QuadSolver.ACCURACY_FACTOR);
	...
	public void testChallangingRoot() {
		...
		verifyRootsAreZeros("ChallangingRoot", qs, r1, r2, QuadSolver.ACCURACY_FACTOR);
	}
	...
	public void testCatastrophicCancellation() {
		...
		final List<Float> roots = getRoots(qs);
		final StringBuilder msg = verifyRootsAreZerosUnchecked(false, "CatastrophicCancellation", qs,
				roots.get(0).floatValue(), roots.get(1).floatValue(), QuadSolver.ACCURACY_FACTOR);
		if (msg.length() < 1)
	...
	public void testIteration() {
		...
		verifyRootsAreZeros("normal quad root", qs, roots.get(0), roots.get(1), (float) 0.04);

		final List<Float> improvedRoots = getIterativeRoots(qs, (float) (accuracyFactor / 8.0));
		...
		verifyRootsAreZeros("Iteration", qs, improvedRoots.get(0), improvedRoots.get(1), (float) 0.04);
	}
	 ...
	 * @param improvedRoots
	 */
	private void checkRootsDiffer(final float accuracyFactor, final List<Float> roots,
			final List<Float> improvedRoots) {
		boolean echo = false;
		...
		for (int i = 0; i < roots.size(); i++) {
			...
			if (eps > accuracyFactor) {
				...
				Assert.assertEquals("perturbed vs iterative root[" + i + "] differs", roots.get(i),
						improvedRoots.get(i), accuracyFactor);
			}
		}
	 ...
	 */
	private List<Float> getIterativeRoots(final QuadSolver qs, final float tolerance) {
		final List<Float> roots = getRoots(qs);
		...
		Assert.assertEquals("iterative root count bad", roots.size(), iterativeRoots.size());
	...
	private StringBuilder verifyRootsAreZeros(final String label, final QuadSolver qs, final float r1, final float r2,
			final float tolerance) {
		final StringBuilder msg = verifyRootsAreZerosUnchecked(true, label, qs, r1, r2, tolerance);
		if (msg.length() > 0)
	...
	private StringBuilder verifyRootsAreZerosUnchecked(final boolean echo, final String label, final QuadSolver qs,
			final float r1, final float r2, final float tolerance) {
		StringBuilder msg = new StringBuilder();
		float yR1 = qs.evaluate(r1);
		float epsR1 = QuadSolver.computeEPS((float) 0.0, yR1);
		if (epsR1 > tolerance)
			msg.append(" IMP_R1[" + r1 + "]: " + yR1 + " differs from 0.0 by " + epsR1 + ";  tolerance: " + tolerance);
		float yR2 = qs.evaluate(r2);
		float epsR2 = QuadSolver.computeEPS((float) 0.0, yR2);
		if (epsR2 > tolerance) {
			msg.append(" IMP_R2[" + r2 + "]: " + yR2 + " differs from 0.0 by " + epsR2 + ";  tolerance: " + tolerance);

			if (echo)
				System.err.println("\n" + label + " -- a: " + qs.getA() + "; b: " + qs.getB() + "; c: " + qs.getC()
						+ ";    R1: " + r1 + " [" + epsR1 + "],    R2: " + r2 + " [" + epsR2 + "]");
		}
	...
	private QuadSolver getPerturbedQS() {
		...
		if (eps >= QuadSolver.ACCURACY_FACTOR)
			Assert.fail("coeff c too different; expected: " + cq + ",  actual: " + c + ", EPS: " + eps);
