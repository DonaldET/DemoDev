 ...
 * opportunities.
 */
package don.demo.algo.quadsolve;
...
/**
 * Solve classical quadradic equations, using both direct and recursive
 * approaches.
 *
...
public class QuadSolver {
	...
	public List<Float> improve(final float r1Inp, float r2Inp, final float tolerance) {
		...
		if (computeEPS(r1, r2) < tolerance && r1 != 0.0) {
			...
			if (computeEPS((float) 0.0, yR2) < tolerance)
				...
				r2 = bisectionImprove(r2Est, tolerance / (float) 16.0, r2Inp - bound, r2Inp + bound);
			}
		}
	...
	private float bisectionImprove(final float rInp, final float tolerance, final float lowBound,
		...
		while (eps >= tolerance) {
			if (itr >= biLimit) {
				System.err.println("\nWARNING: no bisection convergence after " + biLimit + " iterations");
				System.err.println("Input interval : [" + lowBound + ", " + highBound + "]");
				System.err.println("Output interval: [" + lft + ", " + rgt + "]");
				System.err.println("To Upper: " + toUpperHalf + ";  To Lower: " + toLowerHalf);
	...
	private float improveOneRoot(final float rInp, final float tolerance) {
		...
		while (epsR >= tolerance || epsY >= tolerance) {
			if (itr >= limit)
				throw new IllegalStateException("no convergence after " + itr + " iterations, EPS R: " + epsR);
	...
	public static float computeEPS(final float trueValue, final float observedValue) {
	...
	public String toString() {
		return "[" + getClass().getSimpleName() + " - 0x" + Integer.toHexString(hashCode()) + ";  a: " + a + ",  b: "
				+ b + ",  c: " + c + ",  discriminent: " + discriminent + ",  limit: " + limit + "]";
	}
}
