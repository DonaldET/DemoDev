...
import demo.don.bignumeric.impl.RightShiftSequenceGenerator;
...
public class NiaveAdderTest
{
  ...
  public void testNaiveAdder()
  {
    ...
    if (echo)
    {
      System.err.println("Sequence [n:" + MAX_ITERATIONS + "] Result  : "
          + result);
    ...
    Assert.assertEquals("lastSum differs", expectedLastSum, adder.getLastSum(),
        SP_DELTA);

    final int lastIteration = 25;
    Assert.assertEquals("iteration count differs", lastIteration,
        result.getFinalIteration());
    ...
    Assert.assertEquals("in-exact sum for " + lastIteration,
        generator.correctSum(), actualSum, SP_DELTA);
  }
}
