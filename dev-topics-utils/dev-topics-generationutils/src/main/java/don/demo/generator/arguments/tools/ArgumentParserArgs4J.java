 ...
 * home</a>, <a href='https://github.com/kohsuke/args4j'>source code</a> and <a
 * href='http://args4j.kohsuke.org/sample.html'>for an example</a>.
 * 
 * @author dtrumme
 */
...
public class ArgumentParserArgs4J implements ArgumentParser {
    ...
    public ParseBean parseArgs(final String[] args) {
        ...
        final ParameterBean parameterBean = problem == null ? new ParameterBean(
                srcDir, ArgumentUtils.commaToList(templateList, mask),
                defaultContext, (overrideContextList == null) ? new String[0]
                        : ArgumentUtils.commaToList(overrideContextList, mask),
                dstDir, ArgumentUtils.commaToList(generatedFileList, mask))
                : null;

        return new ParseBean(parameterBean, arguments);
    }
     ...
     *            error message label
     * @param ex
     *            exception causing the parse problem
     * 
     * @return a RuntimeException instance capturing error information
     */
    public RuntimeException handleError(final String[] args,
