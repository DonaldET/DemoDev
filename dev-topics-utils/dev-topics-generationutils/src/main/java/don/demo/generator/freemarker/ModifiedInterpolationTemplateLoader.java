...
/**
 * Modify normal Freemarker interpolation requests; this permits bypassing the
 * normal interpolation request,started by the dollar-sign and curly-brace
 * introduction, and replace it with a special form that will be recognized on
 * post-processing template write.
 * 
 * @author dtrumme
 **/
public class ModifiedInterpolationTemplateLoader extends FileTemplateLoader
    ...
    public ModifiedInterpolationTemplateLoader() throws IOException {
        throw new UnsupportedOperationException(
                "ModifiedInterpolationTemplateLoader() not implemented");
    }

    public ModifiedInterpolationTemplateLoader(final File baseDir)
            throws IOException {
        super(baseDir);
    }

    public ModifiedInterpolationTemplateLoader(final File baseDir,
            final boolean disableCanonicalPathCheck) throws IOException {
     ...
     *            the template file from which normal interpolation markers
     *            should be ignored my Freemarker
     * @param the
    ...
    public Reader getReader(final Object templateSource, final String encoding)
        ...
        String replacedContent = FreemarkerHelper.replaceOpeners(rawContent,
                getOpener(), getReplacement());
        rawContent = null; // Just in case it is really big
        replacedContent = FreemarkerHelper.replaceMagic(replacedContent,
                getMagic(), "$");
